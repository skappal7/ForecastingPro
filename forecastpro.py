...
def forecast_with_prophet(
    df, date_col, value_col, periods, freq, seasonality_mode, yearly, weekly, daily, holidays_df=None
):
    m = Prophet(
...
def holt_winters_forecast(df, date_col, value_col, periods, freq, seasonal="add", seasonal_periods=None):
    ...
    if n < 2 * seasonal_periods:
        ...
        st.warning(
            f"Series length {n} is less than two seasonal cycles ({2 * seasonal_periods}). Using non-seasonal ExponentialSmoothing fallback."
        )
        try:
...
def sidebar_file_upload():
    ...
    if uploaded_file is None:
        st.sidebar.info(
            "You can upload a CSV or Excel file with a date column and a numeric value column. Additional columns (categorical or numeric) can be used as exogenous features or for driver analysis."
        )
...
def metrics_to_html_table(metrics_list):
    ...
    table = (
        ...
        "<th style='text-align:left;padding:8px'>MAPE (pill)</th>"
...
def main():
    ...
    with tabs[0]:
        ...
        st.markdown(
            "**Upload format help:** The app expects a single date column and one numeric target column. Additional columns (categorical or numeric) can be used as exogenous features or for driver analysis."
        )
            ...
            try:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        mode="lines+markers",
                        name="value",
                        marker=dict(color=merged["marker_color"], symbol=merged["marker_symbol"], size=8),
                        hoverinfo="text",
                        hovertext=merged["tooltip"],
                    )
                )
                ...
                if not anoms.empty:
                    fig.add_trace(
                        go.Scatter(
                            ...
                            text=["Anomaly"] * len(anoms),
                            textposition="top center",
                            hoverinfo="text",
                            hovertext=anoms["tooltip"],
                        )
                    )
